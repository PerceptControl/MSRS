...
class Relation {
  ...
  #update_weights({ actor, actored, type }) {
    ...
    this.#clear_action_weights([actor_weight, actored_weight])
  }
  ...
  #compare_action_weights([actor, actored]) {
    ...
    actor.action += 0.1 * actor.global.weight
    actored.action += 0.1 * actored.global.weight
  }
  ...
  #update_deltas(type, [actor, actored]) {
    const global_differnce =
      actor.weight.global.weight / actored.weight.global.weight

    switch (type) {
      case 'report':
        actor.weight.delta -= 1 / global_differnce
        if (actor.weight.action > actored.weight.action)
          actored.weight.delta -= 2
        else actored.weight.delta -= 1 * global_differnce
        break
      case 'commend':
        actor.weight.delta -= 0.4 / global_differnce
        if (actor.weight.action_weight > actored.weight.action_weight)
          actored.weight.delta += 1
        else actored.weight.delta += 0.5 * global_differnce
        break
    }
  }
   ...
   * @param {[WeightData, WeightData]} param0
   */
  #clear_action_weights([actor, actored]) {
    actor.action = 0
    actored.action = 0
  }
}
